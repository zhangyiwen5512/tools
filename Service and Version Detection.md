...
![](./assets/20150513170305634.png)
    ...
    Nmap首先按照您的指示进行端口扫描，然后将所有打开或打开的TCP和/或UDP端口传递给服务扫描模块。 然后，这些端口并行查询
    ...
    5.通常，NULL探针或可能端口探针与服务匹配。由于NULL探针与可能端口探针共享连接，因此在大多数情况下，这使得仅用一个短连接来完成服务检测。使用UDP通常只需要一个数据包。但是，当两者失败时，Nmap将顺序执行其他探针。对于TCP，会建立许多新连接，以避免先前的探针破坏结果。这种情况可能需要一些时间，因为Nmap必须等待大约五秒钟才能得到每个探测的结果，由于慢的网络链接或慢的服务响应。Nmap使用了几种自动技术来加速扫描：
